...
impl DefaultImageResizer {
    fn decode(jpeg_bytes: &[u8]) -> CoreResult<(u32, u32, Vec<u8>)> {
        let mut dec = Decompressor::new()
            .map_err(|e| ImageProcessingError::Raw(format!("Decompressor init failed: {e}")))?;
        let header = dec
...
impl ImageResizer for DefaultImageResizer {
    fn resize<'a>(&self, buffer: &'a [u8], max_border: Option<u32>) -> CoreResult<Cow<'a, [u8]>> {
        ...
        let (w, h, mut rgb_data) = Self::decode(buffer)?;

        if w <= dst_max && h <= dst_max {
            return Ok(Cow::Borrowed(buffer));
        }

        let (dst_w, dst_h) = if w > h {
            (dst_max, (h * dst_max) / w)
        } else {
            ((w * dst_max) / h, dst_max)
        };
        ...
        let jpeg = TLS_COMPRESSOR.with(|c| {
            c.borrow_mut()
