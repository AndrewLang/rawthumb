...
impl DefaultImageRotator {
    ...
    #[inline]
    fn rotate_jpeg_lossless(jpeg: &[u8], orientation: Orientation) -> Result<Cow<'_, [u8]>, String> {
        ...
        log::trace!("Attempting JPEG rotation for orientation {:?}, {:?}", orientation, transform);
    ...
    {
        let img: ImageBuffer<P, Vec<u8>> = ImageBuffer::from_raw(width, height, data)
            .ok_or_else(|| ImageProcessingError::Raw("Failed to construct image buffer for rotation".to_string()))?;
        ...
        encoder
            ...
            .map_err(|e| ImageProcessingError::Raw(format!("Failed to encode rotated JPEG: {e}")))?;
        Ok(out)
    }
}

impl DefaultImageRotator {
    fn rotate_decode_encode(&self, buffer: &[u8], orientation: Orientation) -> CoreResult<Cow<'static, [u8]>> {
        log::info!("Falling back to decode-rotate-encode for orientation {:?}", orientation);
        let cursor = Cursor::new(buffer);
        let decoder =
            JpegDecoder::new(cursor).map_err(|e| ImageProcessingError::Raw(format!("JPEG decode failed: {e}")))?;
        let color_type = decoder.color_type();
        let (width, height) = decoder.dimensions();
        let mut data = vec![0u8; decoder.total_bytes() as usize];
        decoder.read_image(&mut data).map_err(|e| ImageProcessingError::Raw(format!("JPEG decode failed: {e}")))?;

        match color_type {
            ColorType::Rgb8 => {
                self.rotate_and_encode::<image::Rgb<u8>>(data, width, height, orientation, ColorType::Rgb8)
            }
            ColorType::L8 => self.rotate_and_encode::<image::Luma<u8>>(data, width, height, orientation, ColorType::L8),
            ColorType::Rgba8 => {
                self.rotate_and_encode::<image::Rgba<u8>>(data, width, height, orientation, ColorType::Rgba8)
            }
            _ => {
                let img = image::load_from_memory_with_format(buffer, ImageFormat::Jpeg)
                    .map_err(|e| ImageProcessingError::Raw(format!("Failed to decode JPEG for rotation: {e}")))?;
